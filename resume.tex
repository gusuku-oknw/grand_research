...
\section*{概要}
秘密画像共有（Secret Image Sharing; SIS）で分散保存した画像について，平文化せずに知覚ハッシュ pHash による類似画像検索を行う方式を提案する．
...
\begin{itemize}
  \item \textbf{知覚ハッシュ (pHash)}: 画像の低周波成分（DCT: Discrete Cosine Transform 係数）から生成される 64bit のハッシュ値．類似画像間でハッシュ間距離（Hamming 距離）が近くなる性質を持つ．
  \item \textbf{生成アルゴリズム}:
    \begin{enumerate}
...
\subsection{実験設定}
\begin{itemize}
  \item \textbf{データセット}: COCO val2017~\cite{mscoco} から 500 枚を抽出．
