...
\section{関連技術}
本研究の基礎となる Shamir の秘密分散法 \cite{shamir1979} について述べる．
$(k, n)$ 閾値法は，秘密情報 $S$ を $n$ 個のシェア $s_1, \dots, s_n$ に分割する方式である．
任意の $k$ 個以上のシェアを集めれば $S$ を完全に復元できるが，
$k$ 個未満では $S$ に関する情報は一切得られない（情報理学的安全性）．
一般的な SIS はこの性質を利用して画像の機密性を担保するが，
前述の通り検索機能は持たない．
提案法ではこの閾値性を拡張し，シェア収集数に応じて復元できる情報の「質」を変化させることで，
多段階のアクセス制御を実現している．
...
\begin{figure*}[t!]
  \centering
  \includegraphics[width=0.85\linewidth]{output/figures/phash_masked_sis/combined_pipeline.png}
...
\bibitem{shamir1979}
A.~Shamir, ``How to share a secret,'' \textit{Commun. ACM}, vol. 22, no. 11, pp. 612--613, 1979.
\bibitem{mscoco}
T.-Y.~Lin et al.,
