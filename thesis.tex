...
あわせて，後続の評価で用いる実装上の設定（パラメータ，乱数の扱い，指標の算出方法）も本章で明確化する．
...
低周波 $8\times 8$ ブロックの係数から 64bit 指紋を生成する．
DCT/IDCT は DCT-II（および逆変換）を用い，2次元変換は分離可能性に基づき 1次元変換を縦横に適用して実装した．
本研究では画像サイズを $32\times 32$ に固定するため，cosine テーブル（変換行列）を事前計算して使い回し，
...
逆 DCT や 0--255 への正規化で係数の符号が反転すると pHash 一致性が崩れるため，
...
\chapter{実装}
本章では，第\ref{chap:proposed}章で述べた三段階開示方式を，Python 実装として具体化する．
あわせて，後続章の評価で用いる実装上の設定（パラメータ，乱数の扱い，指標の算出方法）も本章で明確化する．
...
低周波 $8\times 8$ ブロックの係数を基準値（中央値）で 2 値化して 64bit の指紋を生成する．
...
\section{pHash 整合ダミー生成の実装}
Level 1（$k_1 \le r < k_2$）では，pHash 計算が参照する低周波成分の符号が
...
pHash 指紋 $b$ は 64bit の整数にパックし，$s_L$ として分散する．
...
検索性能の比較は，平文画像の pHash による検索（plain）と，
...
\section{考察}
本節では，本実験で示せたこと／示せていないことを整理し，安全性評価の範囲と今後の検証課題を明確化する．
