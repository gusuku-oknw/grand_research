...
本研究は，サーバ（クラウドサービス提供者など）を信頼しない（semi-honest：プロトコルには従うが得られる情報から推測を試みる）脅威モデルを想定する．
...
一方で，SISのシェアは暗号化画像のようにランダムに見えるため，シェア画像そのものにpHashを適用しても元画像の類似性は保証されない．
...
一方で，知覚ハッシュは入力とハッシュの照合が反復可能な判定器（オラクル）として悪用され得ること，
...
照合結果（ヒットの有無，距離，順位）の応答を通じて検索機能が反復問い合わせ可能な判定器（オラクル）として機能し，探索・推測の手掛かりを与え得るため，
...
検索機能が反復問い合わせ可能な判定器（オラクル）となり得るという論点がある（第\ref{sec:phash_related}節参照）．
...
検索機能が反復問い合わせ可能な判定器（オラクル）として機能し得るという安全性・運用上の注意点が指摘されている\cite{jain2022usenix_phash}．
...
\begin{itemize}
  ...
  \item 復元には所定数のシェア収集が前提となり，「検索だけしたい」という要求と整合しにくい．
\end{itemize}
...
また，pHash よりも頑健な特徴量の採用や，大規模なデータベースでの実用性検証も進めていく．
