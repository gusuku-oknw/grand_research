...
上記の観点のうち，\textbf{検索精度}および\textbf{ロバスト性}を定量的に評価するための指標として，本実験では Precision@$K$ と Recall@$K$ を用いる．
ここで K は評価する上位件数である．
...
まず，ダミーと原画像の視覚的乖離の程度を定量化するためピーク信号対雑音比（Peak Signal-to-Noise Ratio; PSNR）を算出する（図\ref{fig:mask-mse}）．
