...
\section{提案手法}
\subsection{記号と定義}
入力画像をグレースケール 32$\times$32 に縮小し DCT を取る。低周波 8$\times$8 ブロック $C_{\text{LF}}$ の符号で 64bit の pHash $b \in \{0,1\}^{64}$ を定義し、$b_i = 1$ は正、0 は負の符号を表す。二階層 Shamir は $n{=}5$, $(k_1,k_2)=(2,4)$ とし、シェア数 $r$ に応じた段階開示を次のように定義する:
\[
