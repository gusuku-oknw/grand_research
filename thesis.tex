...
現状，国内の多くの医療機関では院内サーバ（オンプレミス）での管理が一般的であるが，これは管理コストの増大や，単一障害点に起因するデータ消失リスクといった課題を抱えている．
...
そのための有力な技術として，画像を複数のシェアへ分割して分散保持し，所定数（$k$-of-$n$）のシェアが揃った場合にのみ復元を可能とする秘密画像共有（Secret Image Sharing; SIS）が注目されている．
これらは，保管先の分散や合意復元といった運用要件を自然に満たし，単一点の侵害に対する耐性を高める枠組みとして位置づけられる．
しかし，SISによって生成されるシェアはランダムなデータ列であり視覚情報を持たないため，分散ストレージ上に保存された状態では，そのまま画像の類似性を判断して検索（Content-Based Image Retrieval; CBIR）を行うことが原理的に不可能である．
医師や管理者が過去の類似症例（画像）を探そうとした場合，現状では対象となるシェアを一度すべて収集し，原画像を復元してから中身を確認する必要がある．
しかし，「検索」のたびに機密性の高い原画像を復元する運用は，計算コストの増大に加え，本来閲覧権限を持たない管理者に対して不要な情報開示を強いることになり，秘密分散によるセキュリティ上の利点を損なう．


したがって，医療画像のような機微情報の利活用においては，「画像の秘匿性（分散保管）」と「検索の利便性」という相反する要件を両立させる新たな枠組みが求められる．
本研究は，分散保持されたシェア（あるいは部分的な復元情報）のみを用いて，原画像を完全復元することなく類似画像検索を実現することを目的とする．
...
前述の通り，SISのシェアはランダムな値を持つため，シェアにそのままpHashを適用しても元画像の類似性は保証されない．
...
COCO派生データ（最大500クエリ）を用いて評価実験を行った．検索性能は Precision@$K$（検索結果上位 $K$ 件に含まれる正解の割合）と Recall@$K$（正解集合のうち上位 $K$ 件で回収できた割合）で評価した．
元画像では Precision@1=100\%となり，平文pHashと同等の検索結果を維持した．
また，回転・切り抜き等の20種類の派生画像群を正解集合とする評価（ロバスト性評価）においてもPrecision@1=100\%，Precision@5=86.6\%，Precision@10=84.82\%であり，平文との差は小さかった．
...
\section{背景}
医療画像や監視映像，個人写真などの機微画像には，画像内容そのものを広く開示することなく，類似事例を検索して診断・治療方針の検討，臨床教育，研究用途に活用したいという要求がある．実際，医療分野ではContent-Based Medical Image Retrieval（CBMIR/CBIR）が，症例参照型の意思決定支援や教育支援のための基盤技術として位置づけられてきた\cite{muller2020mir,sotomayor2021cbmir,su2007decision_support}．

従来のCBIRは，検索精度と運用容易性を優先し，画像特徴量（埋め込みやハッシュ等）をクラウド上のサーバで保持・照合する構成が一般的である．
しかし本研究では，サーバ管理者による目的外の閲覧や，外部攻撃者によるデータ窃取のリスクを考慮し，サーバを完全には信頼しない脅威モデルを採用する．具体的には，サーバ（またはサーバへの侵入者）は計算手順（プロトコル）には従う一方で，保持データや処理結果から画像内容の推測を試みる脅威（semi-honest ／ honest-but-curious）を想定する．

また，データ管理の観点でも課題がある．国内の多くの医療機関では院内サーバ（オンプレミス）での管理が一般的であるが，これは管理コストの増大や，単一障害点に起因するデータ消失リスクといった課題を抱えている．
一方で，可用性向上を目的としたクラウド移行においては，特定の事業者にデータ原本を委ねることによる情報漏えいやプライバシー侵害の懸念が障壁となっている．

したがって，特定の管理主体に依存せず，複数のサーバへ地理的・管理的に分散して保存することで，可用性と機密性を同時に担保するアプローチが望ましい．
そのための有力な技術として，本研究では画像を秘密画像共有（Secret Image Sharing; SIS）により分散保持するアプローチを前提とする．SISは画像を$n$個のシェアに分割し，所定数（閾値）以上のシェアが揃った場合にのみ復元できる一方，閾値未満では原画像に関する意味情報が得られないよう設計される（閾値型SIS）．
脅威モデルとしては，前述のsemi-honestを想定し\cite{semi_honest_katz}，加えて複数主体が結託して観測情報を統合する可能性も考慮する．
このとき，閾値未満のシェア集合からは原画像を復元できないことを安全性の前提とする．

しかし，SISによって生成されるシェアはランダムなデータ列であり視覚情報を持たないため，分散ストレージ上に保存された状態では，そのまま画像の類似性を判断して検索（CBIR）を行うことが原理的に不可能である．
医師や管理者が特定の症例画像を探そうとした場合，現状では対象となるシェアを一度すべて収集し，原画像を復元してから中身を確認する必要がある．
だが，「検索」のたびに機密性の高い原画像を復元する運用は，計算コストの増大に加え，本来閲覧権限を持たない管理者に対して不要な情報開示を強いることになり，秘密分散によるセキュリティ上の利点を損なう．
この問題を解決するためには，分散保管されたシェアの状態（あるいは部分的な復元状態）を保ったまま，原画像を完全復元することなく類似画像検索を可能にする新たな枠組みが必要である．
...
一方で，知覚ハッシュは，攻撃者が生成した任意のクエリとハッシュとの照合結果（距離や合否）を反復的に取得することで，逆像を推定する判定器（オラクル）として悪用され得ることや，
...
\section{目的}
本研究の目的は，SISで分散保持された機微画像集合に対し，原画像を復元（平文化）せずに類似検索を成立させることである．
類似検索を行う以上，検索用表現（特徴量やハッシュ値など）を比較に用いること自体は避けられない．
しかし，サーバを信頼しない（semi-honest：プロトコルには従うが，観測できた情報から追加推測を試みる）環境では，上記オラクルが検索結果を反射的に，探索・推測し得るため，
...
まず，検索可能性と閲覧可能性を混同せずに制御するための段階開示モデルを提示する．
具体的には，シェア収集数$r$に応じて開示レベルを切り替える枠組みを定義し，
$r<k_1$では検索に資する情報を開示せず（Level 0），$k_1\le r<k_2$では検索のみを許可する出力（Level 1）を開示し，
$r\ge k_2$で初めて復元を許可する（Level 2）という二つの閾値$k_1,k_2$に基づくアクセス構造として定式化した．
これにより，「検索だけ許可したい」という運用要求を，復元許可（閲覧許可）と切り離して扱える．
...
この文脈で，知覚ハッシュは，画像を短いビット列へ写像し，
Hamming距離により高速比較できる点で実装・運用上の利点が大きい．
とりわけpHashは，画像を小サイズに正規化した後にDCTを適用し，低周波成分の符号から
64bit程度のハッシュ値（ビット列）を生成する軽量な知覚ハッシュとして広く知られている\cite{Venkatesan2000,farid2021phash}．
一方で，知覚ハッシュは「類似性を保存する」設計であるため，照合の可否や距離が外部から反復的に観測できる状況では，
一方向性を目的としておらず，「似ているものは近い値になる」よう設計されているため，照合の可否や距離が外部から反復的に観測できる状況では，検索機能がオラクルとして悪用される危険性がある（第\ref{sec:phash_related}節参照）．
...
このようにして得られるpHashは，本来「類似性を保存するハッシュ値（ビット列）」であるため，暗号学的ハッシュのような一方向性や耐推測性を保証するものではない．
このため，照合結果（距離やヒットの有無）を外部から反復的に観測できる環境では，検索機能がオラクルとして悪用されるという安全性・運用上の注意点が指摘されている\cite{jain2022usenix_phash}．

以上を踏まえると，pHashをプライバシー保護設定で用いる場合は，単に「pHashを暗号化して比較する」だけでなく，どの情報をどの段階で開示するか（検索の可否と閲覧の可否をどう分離するか）まで含めて設計する必要がある．
...
参加者（保持者）\(i\in\{1,\dots,n\}\)に配布するシェアは，多項式への入力値（評価点）と出力値（評価値）のペアとして構成される．
ここで評価点として，\(0\)以外の互いに相異なる\(n\)個の元\(x_1, \dots, x_n \in \mathbb{F}_p \setminus \{0\}\)を選ぶ
（本研究では実装の単純性のため \(x_i:=i\) を用いる）．
このとき各参加者\(i\)に配布するシェアは
\begin{equation}
  \mathrm{share}_i := (i, f(i))
  \label{eq:share_calculation}
\end{equation}
である．
すなわち，秘密は多項式の定数項として埋め込まれ，シェアはそのサンプル点として配布される．

任意の\(k\)個のシェア集合を\(S \subseteq \{1,\dots,n\}\)（\(|S|=k\)）とする．
このとき，対応するシェア \(\{(x_i, y_i)\}_{i\in S}\) から次数\(k-1\)以下の多項式は一意に定まり，
Lagrange補間により\(f(0)=s\)を直接計算できる．
Lagrange基底多項式を
\begin{equation}
  \ell_j(x)=\prod_{\substack{m\in S\\ m\ne j}}\frac{x-x_m}{x_j-x_m}
  \quad (\bmod\ p)
\end{equation}
とすると，
\begin{equation}
  f(x)=\sum_{j\in S} y_j\,\ell_j(x)\quad (\bmod\ p)
\end{equation}
であり，特に復元したい秘密は
\begin{equation}
  s=f(0)=\sum_{j\in S} y_j\,\lambda_j\quad (\bmod\ p),
  \qquad
...
秘密に関する相互情報量は0となる\cite{shamir1979}．
...
保持者（サーバ群，あるいは分散保管ノード）にシェアを配布する状況を考える．
検索要求が発生した際，収集可能なシェア数$r$に応じて， 検索に必要な最小限の出力のみを返す（閲覧はさせない）
または，原画像$I$の復元を許可する，という出力制御を行う．



脅威モデルとしては，サーバ（および一部の保持者）はプロトコルには従うが，観測できる情報（保持データ，出力，ログ，アクセスパターン等）から推測を試みるsemi-honest攻撃者を想定する．
...
\begin{itemize}
  ...
  システムが検索のみを許可する段階であり，pHash検索に必要な条件だけを満たす「検索専用出力」を生成して開示する．
  本研究ではこれをpHash整合ダミーと呼ぶ．

  \item \textbf{Level 2（$r\ge k_2$）}：
  システムが復元（閲覧）を許可する段階であり，原画像$I$の復元を可能にする．
\end{itemize}

この三段階は，「画像を三回別々に分割する」という意味ではない．
同じ参加者に対して配布するシェアの中に，検索段階に必要な秘密と復元段階に必要な秘密を同居させ，
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{output/fig_scene_extra_large.png}
...
本研究では，前述のShamir秘密分散法（第\ref{sec:shamir_basics_moved}節）を用いて画像を分散する．
...
これにより，画像をバイト列として扱い，\(p\)未満に収まるよう固定長ブロックへ分割して
各ブロックを整数（\(\mathbb{F}_p\)の元）として分散する運用が容易になる．
...
具体的な手順は次の通りである（処理フローは図\ref{fig:dummy-flow}，途中出力例は図\ref{fig:dummy-top3}参照）：
...
\begin{enumerate}
  ...
  \item 構成した係数行列$\widetilde{C}$に逆DCT（IDCT）を適用してpHash整合ダミー（画像）$\mathrm{dummy}(b)$を得る．
  \item $\mathrm{dummy}(b)$からpHashを再計算し，$b$と一致することを確認する．
  一致しない場合は低周波係数の強調量（マージン）を増やすなどして再生成する．
\end{enumerate}

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{output/pHash_dummy_flow.png}
  \caption{pHash整合ダミー生成の処理フロー（低周波成分の構成から検証ループまで）}
  \label{fig:dummy-flow}
\end{figure}
...
このようにして得られた$\mathrm{dummy}(b)$は「pHash一致」を満たすための検索専用出力であり，
視覚的内容の復元（閲覧可能性）を保証しない点が本質である．
以後の章では，ダミーの視認性低下をピーク信号対雑音比（Peak Signal-to-Noise Ratio; PSNR）やSSIM（Structural Similarity）等の指標で評価し，
...
このように，本システムは閾値到達に応じて「検索専用の入力」と「復元入力」を切り替えることで，
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{output/figures/phash_masked_sis/data00_tuned/psnr.png}
...
本実験では，提案手法の実効性を多角的に検証するため，以下の4つの観点について評価を行う：

\begin{itemize}
  \item 検索精度と時間：
  オリジナル画像（劣化なし）を用いた場合の基本的な検索性能 （Precision/Recall） と処理時間．
  \item ロバスト性：
  圧縮・変形・ノイズなどの画像変換が加わった条件下でも，ダミー画像を用いて正しく検索できるか（検索性能の頑健性）．
  \item 検索の実用性：
  上記の結果を踏まえ，「復元せずに検索する」という本手法の運用が，平文検索と比較して実用的な範囲に収まっているか．
  \item 安全性の課題：
  検索の利便性と引き換えに開示した情報（pHash等）が，セキュリティおよびプライバシーに与える影響と対処．
\end{itemize}
上記の観点のうち，検索精度およびロバスト性を定量的に評価するための指標として，本実験では Precision@$K$ と Recall@$K$ を用いる．
ここで $K$ は評価する上位件数である．
...
派生画像群を正解集合とする評価（ロバスト性評価）では同一元画像から生成された派生画像群を $G(q)$ とする．

前節の検索性能に加え，提案方式の基本挙動（安全性と品質のトレードオフ）を確認するための指標として以下を用いる．
まず，ダミーと原画像の視覚的乖離の程度を定量化するため，画像品質評価の一般的指標であるピーク信号対雑音比（Peak Signal-to-Noise Ratio; PSNR）を算出する．
一般にPSNRが30dB以上であれば視覚的な劣化は許容範囲（圧縮アーティファクト程度）とされるが，
図\ref{fig:mask-mse}の結果から，ダミー画像のPSNRは非常に低く（約10dB），この基準（点線）を大きく下回っている．
これはノイズ画像と同等の水準であり，視覚的に原画像の内容を推測することは困難であると言える．
さらに，ダミー生成の内部状態（低周波構成と補強の効果）を確認するため，
処理途中の $32\times 32$ 画像を出力する（図\ref{fig:dummy-top3}）．

本実験では，COCO val2017 \cite{mscoco} から乱数シード 2025 により 500 枚をサンプリングし，これをクエリ集合として使用した．
また，各原画像から固定パラメータの画像変換を適用し，派生セットを作成した．
派生の種類は original を含めて 20 種とした．
具体的には JPEG（品質75, 品質60, 品質50+サブサンプリング），WebP（品質70），回転$\pm30^\circ$（黒埋め），
...
まず検索精度と時間について検証した．図\ref{fig:search-original-prec} と図\ref{fig:search-original-time} に，
...
また，ロバスト性について検証した．
次に，全 20 派生条件について，正解集合を「同一元画像から生成された派生画像群」とするロバスト性評価を行い，
条件ごとの精度と時間を集計した．
全条件平均として Precision@1=100\%，Precision@5=86.6\%，Precision@10=84.82\%，
...
さらに，検索の実用性について考察する．本実装条件では，「検索段階（$k_1$ 到達）において，平文検索と同等のランキング結果・同等の処理時間を維持できる」ことを確認した．
...
Level 1 の出力が視覚的内容推定に結びつきにくいという経験的評価に依存する部分に分かれる．
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth,keepaspectratio]{output/results/masked_phash_eval_figs/precision_summary.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth,keepaspectratio]{output/results/masked_phash_eval_figs/time_summary.png}
...
\begin{table}[t]
  \centering
  \caption{評価した20種類の変換例（mapping.json）}
  \small
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth,keepaspectratio]{output/results/masked_phash_eval_all_figs/precision_summary_all_variants.png}
  \caption{Precision@1（条件別，plain vs dummy\_k1）}
  \label{fig:search-all-prec}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth,keepaspectratio]{output/results/masked_phash_eval_all_figs/time_summary_all_variants.png}
  \caption{Latency（条件別，plain vs dummy\_k1）}
  \label{fig:search-all-time}
\end{figure}


\chapter{おわりに}
本研究では，pHashにより検索可能なSISフレームワークのための知覚暗号化を設計した．
本研究の成果は以下のように整理できる．
