...
COCO派生データ（最大500クエリ）を用いて評価実験を行った．検索性能は Precision@K（検索結果上位K件に含まれる正解の割合）と Recall@K（正解集合のうち上位K件で回収できた割合）で評価した．
