...
\section{背景}
医療画像や監視映像，個人写真などの機微画像では，画像内容を開示せずに類似事例を検索したいという要求がある．
しかし従来のコンテンツベース画像検索（Content-Based Image Retrieval; CBIR）は，検索精度と運用容易性を優先して，画像あるいは特徴量をサーバ側で平文として保持・処理する構成が多い．
この場合，運用者の閲覧権限やサーバ侵入，処理ログの残存などを経由して，画像内容そのものだけでなく「どの画像がどの画像に似ているか」といった類似関係が漏えいし得る．
そこで本研究では，画像を秘密画像共有（Secret Image Sharing; SIS）により分散保持した画像集合を検索対象とし，原画像を復元（平文化）せずに類似画像検索を実現する状況を想定する．
また脅威モデルとして，サーバ（および一部の保持者）はプロトコルには従うが，得られる情報（保持データや観測可能な計算結果）から推測を試みるsemi-honestを想定する．
さらに，複数主体が結託して観測情報を統合する可能性も考慮しつつ，閾値未満のシェア集合からは原画像を復元できない閾値型SISを前提とする．

一方で，SISの各シェアは暗号化画像のようにランダムに見えるため，平文画像に対して設計された特徴抽出をそのまま適用しても類似性が保持されず，検索精度が大きく低下する．
このため，「秘密分散で保護された状態のまま検索する」ための設計が必要となる．
本研究では，検索の特徴量として知覚ハッシュ（perceptual hash; pHash）を採用する．
pHashは低次元のビット列に写像でき，Hamming距離（XORとビットカウント）で高速に比較できるため，大規模データベースに対しても検索・保存コストを抑えやすい\cite{phash}．
本研究で扱うpHashは，画像を32$\times$32のグレースケールに正規化した後，離散コサイン変換（Discrete Cosine Transform; DCT）を行い，
DCT係数の低周波成分（8$\times$8相当）から得られる符号を2値化して64bitの指紋を生成する軽量特徴量である\cite{phash}．
ただし前述の通り，シェア画像そのものにpHashを適用しても，元画像の類似性は保持されない．
したがって本研究の課題は，SISにより保護された画像集合に対し，検索に必要な情報のみを制御して露出させながら，pHash検索を成立させる方式を設計することである．

\section{目的}
類似画像検索には比較のための情報露出が不可欠であり，「復元する／しない」の二択では運用上不十分である．
復元を許可すれば閲覧（内容推定）を許してしまい，復元を禁止すれば検索そのものが成立しないからである．
そこで本研究は，「検索は許可するが閲覧はさせない」中間状態を閾値で保証し，原画像を復元（平文化）せずに類似画像検索を実現する段階開示方式を確立することを目的とする．

具体的には，総シェア数を$n$，収集したシェア数を$r$とし，検索許可と復元許可に必要なシェア数（閾値）をそれぞれ$k_1$，$k_2$（$k_1<k_2$）とする．
$r \ge k_1$のときは検索のみを許可し，$r \ge k_2$のときに初めて原画像の復元を許可する．
この2段の閾値により開示する出力を切り替える点は，多層（multi-level）／多閾値（multi-threshold）アクセス構造に基づく段階開示として位置づけられる．
なお本研究は，シェア数の増加に伴い復元画像の画質が徐々に向上することを主眼とするprogressive型とは異なり，
「検索専用の出力」と「復元可能な出力」を閾値で離散的に分離する点に特徴がある．
検索のみ許可する段階では，pHash検索に必要な低周波構造のみが一致するように合成したダミー画像（pHash整合ダミー）を出力し，
輪郭・文字・顔など内容推定に資する視覚情報は高周波成分のマスクにより意図的に劣化させることで，閲覧可能性を低下させる．
すなわち，pHash整合ダミーは検索に必要な低周波符号の一致を保証する一方，視覚的内容の復元（閲覧）を保証しない出力である．
このように「検索専用の出力」と「復元可能な出力」を閾値で分離し，平文化を伴わない検索を実現する．

\section{貢献}
本研究の貢献は次の三点である．
第一に，pHash整合ダミーと二段の閾値（$k_1$で検索のみ，$k_2$で復元）を組み合わせた段階開示モデルを設計し，
「pHashによる検索可否」と「内容閲覧可否」を分離して制御する枠組みを提示した．
第二に，pHash整合ダミーを既存のpHash検索処理と同一の入出力形式で扱えるように設計し，
$r \ge k_1$ではダミーを用いた検索，$r \ge k_2$では原画像復元へ切り替える一連の処理系（段階開示を組み込んだSIS検索パイプライン）を実装した．
第三に，COCO派生データを用いて，平文画像とpHash整合ダミーの検索結果および処理時間を比較し，
検索精度の維持と段階開示の有効性を実験により示した．
