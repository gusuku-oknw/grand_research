...
また，データ管理の観点でも課題がある．国内の多くの医療機関では院内サーバ（オンプレミス）での管理が一般的であるが，これは災害時のデータ消失リスクや管理コストの増大といった単一障害点の課題を抱えている．
一方で，可用性向上を目的としたクラウド移行においては，特定の事業者にデータ原本を委ねることによる情報漏えいやプライバシー侵害の懸念が障壁となっている．
...
まず，ダミーと原画像の視覚的乖離の程度を定量化するためピーク信号対雑音比（Peak Signal-to-Noise Ratio; PSNR）を算出する（図\ref{fig:mask-mse}）．図\ref{fig:mask-mse}の結果から，ダミー画像のPSNRは非常に低く（約10dB程度），ノイズ画像と同等の低い値を示しており，視覚的に原画像の内容を推測することは困難であると言える．
