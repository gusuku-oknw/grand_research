...
def generate_all_plots(
    metrics_csv: Path,
    output_dir: Path,
    exclude_modes: Sequence[str] | None = None,
) -> None:
    records = load_metrics_csv(metrics_csv)
    if exclude_modes:
        records = [rec for rec in records if rec.mode not in set(exclude_modes)]
...
def main() -> None:
    ...
    parser.add_argument("--output_dir", type=Path, default=Path("output/figures"))
    parser.add_argument(
        "--exclude-mode",
        type=str,
        nargs="+",
        default=[],
        help="Exclude these modes from the generated figures (e.g., aes_gcm)",
    )
    args = parser.parse_args()

    generate_all_plots(args.metrics_csv, args.output_dir, exclude_modes=args.exclude_mode)
