...
このリポジトリの主要な研究トラックは `phash_masked_sis/` ディレクトリにあり、低閾値では元画像と同じ pHash を持つマスク（ダミー）画像を復元し、高閾値でのみ本物画像を復元する二階層 Shamir 構成を検証しています。
...
### `phash_masked_sis` の内部処理メモ
- オルソ正規化した DCT-II を `(hash_size * highfreq_factor)`（デフォルト 32×32）のグレースケールに適用し、左上 `hash_size×hash_size` の低周波ブロックの平均で符号を決めて 64bit pHash を得る。
- ダミー生成では、その低周波ブロックを目標ビットの符号にマージン付きで設定し、数回強調して補間・量子化でも符号が反転しないようにする。高周波はガウスノイズを制御して付与し、IDCTで空間に戻して元サイズへアップサンプル。
- 2階層 Shamir: `k1` で pHash一致ダミー（視覚ノイズ）、`k2` で本物、`<k1` は純ノイズ。閾値ごとの漏洩・再構成挙動を測れるようにしている。
- ツール類: `experiments/scripts/evaluate_phash_masked_sis.py`（メトリクス）、`plot_phash_masked_sis_eval.py`（論文向け図版）、`visualize_phash_masked_sis_image.py`（単一画像のパイプライングリッドと pHash 距離）。
