"""
Run a tiny searchable-SIS demo on data/tests and save a JSON summary.

Usage (defaults are tuned for the small data/tests set):
    python scripts/run_demo_tests_summary.py
"""
...
def run_demo(
    images_dir: Path = Path("data/tests"),
    shares_dir: Path = Path("out/demo_tests_shares"),
    meta_dir: Path = Path("out/demo_tests_meta"),
    recon_dir: Path = Path("out/demo_tests_recon"),
    summary_path: Path = Path("out/demo_tests_summary.json"),
    k: int = 2,
    n: int = 3,
    bands: int = 8,
...
if __name__ == "__main__":
    summary = run_demo()
    print(f"Saved summary to out/demo_tests_summary.json")
